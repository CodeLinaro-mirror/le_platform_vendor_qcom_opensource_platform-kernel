...
int virt_rsm_txbuf(struct virtio_rsm_txbuf *send_buf)
{
        ...
        virtqueue_add_outbuf(g_vdevrsm->vq_tx, sg, 1, cpu_addr, GFP_KERNEL);
        if (err) {
...
static void virtio_rsm_recv_cb(struct virtqueue *vq_rx)
{
        ...
        unsigned long flags;
        buf = kzalloc(sizeof(struct virtio_rsm_rxbuf), GFP_KERNEL);
        unsigned int len;
...
static int virtio_rsm_probe(struct virtio_device *vdev)
{
	...
	for (i = 0; i < vdev_rsm->num_buf; i++) {
		...
		err = virtqueue_add_inbuf(vdev_rsm->vq_rx, &sg, 1, cpu_addr,
				GFP_KERNEL);
